...
struct total_equals {
  static constexpr const auto value = (sizeof...(Args) == N);
};

template <typename T, typename... Args>
struct count_same {
  static constexpr const auto value = ((... + (std::same_as<T, Args> ? 1 : 0)));
};
...
template <typename T, typename... Ts>
using all_convertible = std::conjunction<std::is_convertible<T, Ts>...>;
